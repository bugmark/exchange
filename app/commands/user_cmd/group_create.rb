module UserCmd
  class GroupCreate < ApplicationCommand
    ...
    def initialize(xargs)
      ...
      add_event :tracker, Event::GroupCreated.new(tracker_opts(@args))
    end
    ...
    def valid_tracker_type
      ...
      return true if TYPES.map {|x| "Group::#{x}"}.include?(@args["type"])
      errors.add "type", "must be one of (#{TYPES.join("|")})"
      return false
    end

    def tracker_type(type)
      return type if TYPES.map {|x| "Group::#{x}"}.include?(type)
      "Group::#{type}"
    end
